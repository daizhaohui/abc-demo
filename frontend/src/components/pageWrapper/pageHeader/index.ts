  ...
  setup (props, context) {
    ...
    onBeforeMount(() => {
      ...
      if (GlobalState.Menu.menuId) {
        data.breadcrumbs = breadcrumbs.concat(menuSerice.getBreadcrumbs(GlobalState.Menu.menuId)).concat(props.breadcrumbs || []);
      } else {
        if (route.meta.auth) {
          data.breadcrumbs = breadcrumbs.concat(menuSerice.getBreadcrumbs(route.meta.auth)).concat(props.breadcrumbs || []);
        }
      }
    });
